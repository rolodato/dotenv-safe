...
function MissingEnvVarsError(allowEmptyValues, dotenvFilename, sampleFilename, missingVars, error) {
  ...
  this.message = 'The folowing variables are defined in ' + sampleFilename +
    ' but are not defined in the environment: ' + missingVars.join(', ') + '.\n' +
    'Make sure to add them to ' + dotenvFilename + ' or directly to the environment.';
  if (!allowEmptyValues) {
    var emptyValuesMessage = 'If you expect any of these missing variables to be empty,' +
      ' you can use the allowEmptyValues option: \n\n' +
  ...
  if (error) {
    var errorMessage = 'Also, the following error was thrown when trying to read variables from '
      + dotenvFilename + ': ' + error.message;
    this.message += '\n' + errorMessage;
  }
}
